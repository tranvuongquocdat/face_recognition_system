...
class UpdateFrameThread(QThread):
    ...
    def __init__(self, cap, model, parent=None):
        super().__init__(None)
        self.cap = cap
        self.running = True
        self.window = parent
    ...
    def run(self):
        while self.running:
            ...
            if ret:
                ...
                people = self.model.find(frame, distance_threshold)
...
class AttendanceCheckingWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.thread_frame = UpdateFrameThread(self.cap, self.deepface, self)
    ...
    def update_frame_logic(self, people, ret, frame):
        self.update_frame_cascade_logic([], frame)
        if len(people) > 0:
            try:
