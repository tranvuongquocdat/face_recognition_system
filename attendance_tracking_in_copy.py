...
class AttendanceCheckingWindow(AttendanceCheckingWindowUI):  # Inherit from the UI class
    ...
    def load_images_from_database(self):
        # Fetch all records from the user_images table in the online database
        ...
        for result in results:
            user_id = result[1]  # Access 'user_id'
            images = result[2]   # Access 'images' (list of base64 strings)
            image_id = result[0]  # Access 'image_id' for unique identification
            ...
            # Flag to check if any new image has been created
            folder_has_images = False

            # Iterate over each base64-encoded image and check if it needs updating
            for index, image_base64 in enumerate(image_base64_list):
                if len(image_base64) > 0:
                    # Construct image file path using image_id and index
                    image_filename = f"{image_id}_{index}.png"
                    image_path = os.path.join(user_folder_path, image_filename)

                    # Check if the image file already exists
                    if not os.path.exists(image_path):
                        # If the image file doesn't exist, decode and save it
                        image_data = base64.b64decode(image_base64)
                        with open(image_path, 'wb') as image_file:
                            image_file.write(image_data)
                        print(f"Created new image: {image_filename}")
                        folder_has_images = True  # Mark folder as having images
                    else:
                        # Optionally, you can add additional checks here if you want to verify
                        # the contents of the existing file and update if necessary.
                        print(f"Image already exists, skipping: {image_filename}")
                        folder_has_images = True  # Folder has images if file exists

            # After processing images for the user, check if the folder is empty
            if len(os.listdir(user_folder_path)) == 0:
                os.rmdir(user_folder_path)  # Remove the folder if it is empty
                print(f"Removed empty folder: {user_folder_path}")

        print("Updated image folders from online database and removed empty folders")
